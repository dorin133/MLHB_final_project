...
#   results = dict((user_type,{'accuracy': 0, 'precision': 0}) for user_type in user_types)
  ...
  for user_type in user_types:
    # if user_type in  ['Rational', 'Attraction'] :
    #     continue
...
def recommend_items_wraper(X_train, X_test, y_train, y_test, model_init, num_items_to_recom, varnames =['x_0', 'x_1', 'x_2', 'x_3','x_4'],
                           user_types = ['Rational', 'Compromise', 'Similarity', 'Attraction']):
    recommendations_features_curr_model = {}
    # non-sklearn wrraped models:
    if type(model_init) == 'MyRanker':
        models_per_user, _ = train_pairwise_ranker_models(X_train, X_test, y_train, y_test, model)
        ...
        models_per_user = train_models_per_user(X_train, X_test, y_train, y_test, model_init, varnames=varnames)
        for user_name in user_types:
            # if user_name in  ['Rational', 'Attraction'] :
            #     continue
            recommendations_features_curr_model[user_name] = recommend_items(models_per_user[user_name], X_test, varnames=varnames, num_items_to_recom = num_items_to_recom)
